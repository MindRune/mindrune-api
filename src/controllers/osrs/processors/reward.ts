...
// Define interfaces for the reward item structure
 ...
 */
export const batchProcessRewards = async (
  ...
  logger.info(`Processing ${rewardEvents.length} rewards`);
  ...
  const itemsParams = {
    ...
    events: rewardEvents.flatMap(event => {
      ...
      const items: RewardItem[] = event.details.items || [];
      return items.map((item: RewardItem) => ({
        rewardId: event.details.rewardId,
        itemId: item.itemId,
  ...
  if (itemsParams.events.length > 0) {
    logger.info(`Processing ${itemsParams.events.length} items from rewards`);
...
export default batchProcessRewards;