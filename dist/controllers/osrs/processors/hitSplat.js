...
exports.batchProcessHitSplats = void 0;
...
const batchProcessHitSplats = async (session, events, txnUuid, dataUuid, account, playerId) => {
    ...
    if (events.length > 0) {
        ...
        logger_1.default.info(`Sample HIT_SPLAT event: ${JSON.stringify({
    ...
    logger_1.default.info(`Processing ${events.length} HIT_SPLAT events: ${incomingEvents.length} incoming, ${outgoingEvents.length} outgoing, ${unknownEvents.length} unknown`);
    ...
    if (incomingEvents.length > 0) {
        ...
        if (incomingParams.events.length > 0) {
            logger_1.default.info(`Sample processed incoming event: ${JSON.stringify(incomingParams.events[0])}`);
        }
            ...
            events: outgoingEvents.map(event => {
                ...
                let target = event.details.target ?
                    (typeof event.details.target === 'string' ?
                        ...
                        String(event.details.target)) : 'Unknown';
    ...
    if (unknownEvents.length > 0) {
        const unknownParams = {
            ...
            }).filter(event => event.target && event.target !== '' && event.target !== 'Player' && !event.isTargetDamageType) // Filter invalid targets
        };
        // Only proceed if we have valid events after filtering
...
exports.batchProcessHitSplats = batchProcessHitSplats;
exports.default = exports.batchProcessHitSplats;
//# sourceMappingURL=hitSplat.js.map